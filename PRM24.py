...
def fin_destination(path):
    course = []
    newnode = cal_dist_and_angle(path[0], path[1])

    for i in range(path):
        if cal_dist_and_angle(newnode, path[i])[1]
        newnode.append cal_dist_and_angle(newnode, path[i])
